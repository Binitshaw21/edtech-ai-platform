...
function switchTab(activeTab, activeSection) {
    ...
    [sectionMic, sectionFile, sectionLink].forEach(sec => sec.classList.add('hidden'));
...
// --- 1. Microphone Recording Logic (WITH WAVEFORM VISUALIZER) ---
let mediaRecorder;
let audioChunks = [];
...
async function startRecording() {
    try {
        ...
        recordBtn.className = "bg-gray-700 hover:bg-gray-600 text-white font-bold py-4 px-8 rounded-full shadow-lg transition-all flex items-center gap-2 text-lg";
        recordBtn.innerHTML = `<i class="fa-solid fa-stop text-red-500"></i> Stop Recording`;
...
function stopRecording() {
    ...
    recordBtn.className = "bg-red-600 hover:bg-red-700 text-white font-bold py-4 px-8 rounded-full shadow-lg transition-all flex items-center gap-2 text-lg";
...
uploadFileBtn.addEventListener('click', async () => {
    if (!fileInput.files.length) return showToast("Please select a file first!");
    
    const file = fileInput.files[0];
    const formData = new FormData();
    formData.append('audio', file, file.name); 

    showProcessing("Whisper AI is processing your file... (This may take a minute)");
    await uploadMedia('/upload-audio', formData);
});
...
processLinkBtn.addEventListener('click', async () => {
    ...
    if (!url) return showToast("Please paste a valid video link first!");

    showProcessing("Downloading and transcribing audio from the link... (Please wait)");
...
// --- 4. GENERATE AI STUDY PACK LOGIC ---
async function generateStudyPack(noteId, buttonElement) {
    // Show loading state on the button
    const originalText = buttonElement.innerHTML;
    buttonElement.innerHTML = `<i class="fa-solid fa-circle-notch fa-spin"></i> Analyzing...`;
    ...
    try {
        const response = await fetch(`/generate-study-pack/${noteId}`, { method: 'POST' });
        const result = await response.json();
        
        if (result.success) {
            showToast("Study Pack Ready! Reloading...", "success");
            setTimeout(() => window.location.reload(), 1500); // Reload to show the new cards
        } else {
...
function showToast(message, type = 'error') {
    ...
    const icon = type === 'error' ? 'fa-circle-exclamation' : 'fa-wand-magic-sparkles';

    toast.className = `glass-card border px-5 py-4 rounded-xl flex items-center gap-3 shadow-2xl transform transition-all duration-500 translate-x-[120%] opacity-0 backdrop-blur-md pointer-events-auto max-w-sm ${styleClass}`;
    toast.innerHTML = `<i class="fa-solid ${icon} text-xl"></i> <p class="font-medium text-sm leading-tight">${message}</p>`;
